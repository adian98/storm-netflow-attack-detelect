 ...
 * 针对NetFlow第二版解析cached过多的问题进行代码修改
 * 原始接受二进制流数据的命令, 在二进制目录下按照日期进行文件的存储
 *
 */
import java.io.File;
...
public class NetflowParser3 {
    ...
    public static void parseAndCleanup(String inDir, String tmpDir, String outDir, NetflowParser3 p) throws Exception{
        ...
        for(File file : target){
            if(file.isFile() && file.getName().substring(0, 6).equals("nfcapd")){
                ...
                String cmd2 = "cp " + tmpOutputDir + " " + outputDir;
                Process process2 = Runtime.getRuntime().exec(new String[]{"bash","-c", cmd2});
                process2.waitFor();

                Calendar calendar = Calendar.getInstance();
                String date = sdf1.format(calendar.getTime());
                String outTarget = outDir + date ;
                File outTargetFile = new File(outTarget);
                if(!outTargetFile.exists() && !outTargetFile.isDirectory()){
                    outTargetFile.mkdir();
                }
                String outputDir1 = outTarget + "/" + file.getName() + ".txt";
                String cmd3 = "mv " + tmpOutputDir + " " + outputDir1;
    ...
    public static void main(String[] args) throws Exception {
        ...
        NetflowParser3 p = new NetflowParser3();
